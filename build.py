...
# ══════════════════════════════════════════════════════════════════════
#  MINIMAL VALID DEX (stub for APK structure)
# ══════════════════════════════════════════════════════════════════════

def make_minimal_dex():
    """Minimal valid DEX v035 with zero classes — structurally valid stub."""
    import hashlib
    header_size = 112
    endian = 0x12345678
    file_size = header_size
    # Build 112-byte header manually
    # Offsets: magic(8) checksum(4) sha1(20) file_size(4) header_size(4)
    #          endian(4) link_size(4) link_off(4) map_off(4)
    ...
    #          data_size(4) data_off(4)  = 8+4+20+4*20 = 112 bytes
    header = bytearray(header_size)
    header[0:8]   = b'dex\n035\x00'
    header[32:36] = struct.pack('<I', file_size)
    header[36:40] = struct.pack('<I', header_size)
    header[40:44] = struct.pack('<I', endian)
    # Compute SHA1 over bytes 32+ (skip magic, checksum, sha1 fields)
    dex = bytes(header)
    sha1 = hashlib.sha1(dex[32:]).digest()
    dex = dex[:12] + sha1 + dex[32:]
    # Compute Adler-32 checksum over bytes 12+
    MOD, a, b = 65521, 1, 0
    for byte in dex[12:]:
        a = (a + byte) % MOD
        b = (b + a) % MOD
    checksum = (b << 16) | a
    dex = dex[:8] + struct.pack('<I', checksum) + dex[12:]
    return dex
